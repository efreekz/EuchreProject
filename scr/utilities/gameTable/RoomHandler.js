...
class RoomHandler {
    ...
    async handleJoinedRoom(e) {
        ...
        console.log('joinedRoom string', data);
        if (typeof e === 'string') {
            data = JSON.parse(e);
        }
        ...
        console.log('joinedRoom json', data);
        ...
        if (data?.roomId) {
            ...
            if (findedRoom) {
                if (players.length >= 4 && findedRoom?.roomType !== 'tournament') {
                    ...
                    const deductionTime = new WalletTransactionService(findedRoom?.teamOne, findedRoom?.teamTwo, gameDetails?.entry, gameDetails)
    ...
    async initializeRoom(findedRoom, data) {
        ...
        const shufflecards = new shuffleCards(1);
